...
export default function MobileNavClient({ navItems, onMenuOpenChange, children, notificationBadge }: {
  navItems: NavItem[];
  currentPath?: string;
    ...
    <>
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <motion.div
              className="fixed left-0 top-0 h-full w-64 shadow-lg flex flex-col"
              style={{ background: 'var(--color-surface)', color: 'var(--color-text)' }}
            ...
            >
              ...
              {/* Bottom: search/inbox/notifications + profile + logout */}
              <div className="p-4 space-y-2 border-t" style={{ borderColor: 'var(--color-border)' }}>
                ...
                {children}
                <motion.button
                  className="w-full py-2 bg-accent hover:opacity-90 text-white rounded-lg transition-opacity"
