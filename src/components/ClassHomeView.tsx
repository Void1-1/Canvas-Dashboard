...
export default function ClassHomeView({ course, announcements = [], assignments = [] }: { course: any; announcements?: any[]; assignments?: any[] }) {
  ...
  // Determine if there is a homepage link
  const homepageUrl = course.home_page_url || (course.default_view === 'wiki' && `/classes/${course.id}/modules`);
  ...
  return (
    <PageTransition>
      <motion.div className="space-y-8 md:space-y-12" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6 }}>
        ...
        <motion.div className="glass p-6 md:p-10 rounded-xl shadow" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }} style={{ background: 'var(--color-surface)' }}>
