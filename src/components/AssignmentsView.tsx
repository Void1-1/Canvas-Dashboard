...
import BackToNotifications from "./BackToNotifications";
...
export default function AssignmentsView({
  ...
  // Auto-open a specific assignment when deep-linked from notifications
  useEffect(() => {
    if (!openId || !openCourseId) return;
    let cancelled = false;
    (async () => {
      try {
        const res = await fetch(`/api/assignments/${openId}?course_id=${openCourseId}`);
        ...
        // silently ignore — user can still browse the list
      }
    })();
  ...
  const handleViewChange = (mode: ViewMode) => {
    ...
    // Clear open params when switching views
    params.delete('open');
    params.delete('course_id');
    router.push(`?${params.toString()}`);
  };

  const isOverdue = (assignment: any): boolean => {
    if (!assignment.due_at) return false;
    const due = new Date(assignment.due_at);
    const now = new Date();
    const state = assignment.submission?.workflow_state;
    const isUnsubmitted = !state || state === 'unsubmitted';
    return due < now && isUnsubmitted;
  };
  ...
  const handleAssignmentClick = async (assignment: any, course: any) => {
    ...
    if (assignment.id && course.id) {
      try {
        ...
        // fall through to basic assignment object
      }
    }
    setLoadingAssignmentId(null);
        ...
        <div className="flex items-center gap-2">
          {(['upcoming', 'past', 'all'] as ViewMode[]).map((mode) => (
            <button
              ...
              style={{
                color: viewMode === mode ? 'var(--color-accent)' : 'var(--color-muted)',
                borderColor: viewMode === mode ? 'var(--color-accent)' : 'var(--color-border)',
                background: viewMode === mode ? 'rgba(var(--color-accent-rgb, 99,102,241), 0.08)' : 'transparent',
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-start gap-3">
                            ...
                            {/* Assignment details */}
                            <div
                              className={`flex-1 cursor-pointer hover:opacity-80 transition-opacity ${loadingAssignmentId === String(assignment.id) ? 'opacity-60 pointer-events-none' : ''}`}
                              ...
                              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2">
                                <div className="flex-1">
                                  <div className="flex items-center gap-2">
                                    ...
                                    {loadingAssignmentId === String(assignment.id) && (
                                      <svg className="w-4 h-4 animate-spin flex-shrink-0" style={{ color: 'var(--color-accent)' }} fill="none" viewBox="0 0 24 24">
                                ...
                                <div className="flex flex-col sm:items-end gap-1">
                                  {assignment.due_at && (
                                    <span
                                      ...
                                      style={{
                                        color: isOverdue(assignment)
                                          ? 'rgb(220,38,38)'
                                          : "var(--color-muted)",
                                        fontWeight: isOverdue(assignment) ? 600 : 400,
                                      }}
                                    >
