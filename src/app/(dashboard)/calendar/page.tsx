...
export default async function CalendarPage() {
  ...
  try {
    ...
    const processedUpcoming = upcoming.map((event: any) => {
      ...
      let type = 'event';
      if (plannableType) {
        const pt = plannableType.toLowerCase();
        if (pt.includes('assignment')) type = 'assignment';
        else if (pt.includes('quiz')) type = 'quiz';
        else if (pt.includes('announce')) type = 'announcement';
        else if (pt.includes('wiki') || pt === 'page') type = 'wiki_page';
        else if (pt.includes('submission')) type = 'submission';
        else if (pt.includes('discussion')) type = 'discussion';
        else if (pt.includes('event') || pt.includes('calendar')) type = 'event';
        else type = pt;
      }
      ...
      return {
        // Spread original event first so computed/normalized fields below take precedence.
        // Previously ...event was last, which overwrote context_name, assignment_id, course_id
        // etc. with raw undefined/null values and broke deduplication.
        ...event,
