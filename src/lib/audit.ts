...
export function logAuditEvent(event: AuditEvent) {
  ...
  // Always log to stdout for centralized log aggregators.
  // Never include secrets (passwords, tokens) in this payload.
  console.log('[audit]', line);

  // If AUDIT_LOG_FILE is set, also append to a local file for persistent storage.
  ...
  if (logFile) {
    try {
      ...
      console.error('[audit] Failed to write to AUDIT_LOG_FILE:', err);
    }
  }
}
